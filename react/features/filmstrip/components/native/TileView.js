...
import { Icon, IconArrowLeft, ArrowRight, IconAdd } from '../../../base/icons';
...
class TileView extends PureComponent<Props> {
    ...
    render() {
       ...
       const _goToNextPage = () => {
            ...
            if (activePage >= participants.length - 1) {
                newPage = 0
            }
            this.pagerViewRef.setPage(newPage)
        }
       const _goToPrevPage = () => {
            const { activePage } = this.state;
            var newPage = activePage - 1;
            if (activePage <= 0) {
                newPage = participants.length - 1
            }
            this.pagerViewRef.setPage(newPage)
        }
        ...
        return (
            ...
            <SafeAreaView style={[styles.flatListContainer, {
                ...
                <PagerView ref={o => this.pagerViewRef = o} showPageIndicator style={styles.pagerViewMaster} initialPage={0}
                    onPageSelected={this._onpageselected}>
                    {participants.map((item, index) => {
                        const { _thumbnailHeight } = this.props;

                        return (
                            <TouchableOpacity key={index} style={styles.perPage} activeOpacity={1}
                                onPress={onClick}
                            >
                                <View >
                                    <Thumbnail
                ...
                <Text style={{
                    ...
                    {this.state.activePage+1}/{participants.length}
                </Text>
            </View>
            </SafeAreaView>
        );
